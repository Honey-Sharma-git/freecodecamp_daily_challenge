import { BMI_ERROR_MESSAGES, BMI_MULTIPLIER, DETECT_MUTATION_ERROR_MESSAGES, LEET_CODES, LOOKUP_TABLE, } from "../../constants/config";
...
export function detectMutations(strand1, strand2) {
    ...
    if (strand1.trim().length !== strand2.trim().length) {
        throw new Error(DETECT_MUTATION_ERROR_MESSAGES.STRAND1_STRAND2_SHOULD_BE_OF_EQUAL_LENGTH);
    }
    ...
    try {
        ...
        throw new Error(DETECT_MUTATION_ERROR_MESSAGES.STRAND1_CONTAINS_INVALID_CHARACTERS);
    }
    ...
    try {
        ...
        throw new Error(DETECT_MUTATION_ERROR_MESSAGES.STRAND2_CONTAINS_INVALID_CHARACTERS);
    }
...
export function isStrandValid(strand) {
    ...
    for (const char of strand.trim()) {
        ...
        if (!isValidCharPresent) {
            throw new Error(DETECT_MUTATION_ERROR_MESSAGES.STRAND_CONTAINS_INVALID_CHARACTERS);
        }
    }
    return isValid;
}
...
export function capitalizeWord(word) {
    const result = word?.trim()?.at(0)?.toUpperCase() + word?.trim()?.slice(1)?.toLowerCase();
    return result;
}
    ...
    for (const w of lookUpMap) {
        ...
        if (regExp.test(str)) {
            str = str.replace(regExp, (match) => {
                if (match === capitalizeWord(w[0]))
                    return capitalizeWord(w[1]);
                return w[1];
            });
        }
    }
